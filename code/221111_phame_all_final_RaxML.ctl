refdir =../data/ref # directory where reference (Complete) files are located
  workdir = ../data/221111_phame_all_final_RaxML # directory where contigs/reads files are located and output is stored

reference = 0  # 0:pick a random reference from refdir; 1:use given reference; 2: use ANI based reference
reffile =  # reference filename when option 1 is chosen

project = 221111_Apul_final_RaxML  # main alignment file name
...
PosSelect = 0 # 0:No; 1:use PAML; 2:use HyPhy; 3:use both # these analysis need gff file to parse genomes to genes
  ...
  threads = 14  # Number of threads to use

   cutoff = 0.001  # Linear alignment (LA) coverage against reference - ignores SNPs from organism that have lower cutoff.
